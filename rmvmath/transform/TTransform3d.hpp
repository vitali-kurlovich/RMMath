...
public:
    ...
    const rmmath::matrix::TAffineMatrix4x4<T> getTransform() noexcept {
        if (isTransformNeedsUpdate()) {
            _localTransform = rmmath::affineTRSMatrix4x4<T>(_localPosition, _localRotation, _localScale);
            setTransformUpdated();
        }
    ...
    void setTransform(const rmmath::matrix::TAffineMatrix4x4<T> &transform ) {
        ...
        rmmath::convertTRSTransform(_localTransform, &_localPosition, &_localRotation, &_localScale);
        setTransformUpdated();
    }
